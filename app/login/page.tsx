...
function LoginContent() {
  ...
  useEffect(() => {
    ...
    // URL上のerror=Configurationパラメータをクリーンアップ
  ...
  // 動的にフォームを作成してsubmit（URLをDOMに一切残さない）
  const handleLogin = async (provider: "apple" | "google" | "github") => {
    ...
    try {
      ...
      // 2. フォームを動的生成（URLはこの瞬間までDOMに存在しない）
  ...
  const handleNativeGoogle = () => {
    window.open(
      `https://www.lattice-protocol.com/api/auth/signin/google?callbackUrl=${encodeURIComponent(
