...
const CREDIT_PLANS = [
  { id: "credits_100", label: "100", price: "500", popular: false },
  { id: "credits_500", label: "500", price: "2,000", popular: true },
  { id: "credits_1000", label: "1,000", price: "3,500", popular: false },
];

const SUB_PLANS = [
  { id: "personal", label: "Personal", price: "980", features: ["300cr / 月 (150回実行)", "エージェント無制限", "Gmail / Discord 連携", "スケジュール実行"] },
  { id: "business", label: "Business", price: "4,980", features: ["1,500cr / 月 (750回実行)", "エージェント無制限", "全連携 (LINE含む)", "優先サポート"] },
];

const CS = { background: "#1c2028", border: "1px solid #2e3440", borderRadius: 12, padding: "20px", marginBottom: 12 };
const SL: React.CSSProperties = { fontSize: 11, color: "#6a7080", letterSpacing: "0.06em", textTransform: "uppercase", margin: "0 0 14px" };
    ...
    return (
      <main style={{ minHeight: "100vh", backgroundColor: "#0e1117", color: "#e8eaf0", paddingBottom: 100 }}>
        <div style={{ maxWidth: 420, margin: "0 auto", padding: "48px 20px 24px" }}>
          ...
          <p style={SL}>クレジット購入</p>
          <h1 style={{ fontSize: 24, fontWeight: 700, color: "#f0f2f8", margin: "0 0 6px", letterSpacing: "-0.02em" }}>クレジットを追加する</h1>
          <p style={{ fontSize: 13, color: "#6a7080", margin: "0 0 28px" }}>残高: <span style={{ color: "#e8eaf0", fontWeight: 600 }}>{credits} クレジット</span></p>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {CREDIT_PLANS.map((p) => (
              <div key={p.id} style={{ background: p.popular ? "#14163a" : "#1c2028", border: `1px solid ${p.popular ? "#6c71e8" : "#2e3440"}`, borderRadius: 12, padding: "20px", position: "relative" }}>
                {p.popular && <span style={{ position: "absolute", top: -10, left: 16, fontSize: 11, color: "#fff", background: "#6c71e8", padding: "2px 10px", borderRadius: 20, fontWeight: 600 }}>人気</span>}
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                  <p style={{ fontSize: 18, fontWeight: 700, color: "#f0f2f8", margin: 0 }}>{p.label} cr</p>
                  <p style={{ fontSize: 22, fontWeight: 700, color: "#f0f2f8", margin: 0 }}>&yen;{p.price}</p>
                </div>
    ...
    return (
      <main style={{ minHeight: "100vh", backgroundColor: "#0e1117", color: "#e8eaf0", paddingBottom: 100 }}>
        <div style={{ maxWidth: 420, margin: "0 auto", padding: "48px 20px 24px" }}>
          ...
          <p style={SL}>プラン</p>
          <h1 style={{ fontSize: 24, fontWeight: 700, color: "#f0f2f8", margin: "0 0 6px", letterSpacing: "-0.02em" }}>プランをアップグレード</h1>
          <p style={{ fontSize: 13, color: "#6a7080", margin: "0 0 28px" }}>現在のプラン: <span style={{ color: "#e8eaf0", fontWeight: 600 }}>{planLabel}</span></p>
                ...
                <div key={p.id} style={{ background: p.id === "business" ? "#14163a" : "#1c2028", border: `1px solid ${p.id === "business" ? "#6c71e8" : "#2e3440"}`, borderRadius: 16, padding: "24px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 16 }}>
                    ...
                    <div style={{ textAlign: "right" }}>
                      <span style={{ fontSize: 28, fontWeight: 700, color: "#f0f2f8" }}>&yen;{p.price}</span>
                  ...
                  <button onClick={() => !isCurrent && handlePurchase(p.id)} disabled={isCurrent || purchasing === p.id} style={{ width: "100%", padding: "12px", borderRadius: 10, border: "none", background: isCurrent ? "#242830" : "#6c71e8", color: isCurrent ? "#6a7080" : "#fff", fontSize: 14, fontWeight: 600, cursor: isCurrent ? "default" : "pointer", fontFamily: "inherit", opacity: purchasing === p.id ? 0.5 : 1 }}>
                    {isCurrent ? "現在のプラン" : purchasing === p.id ? "処理中..." : "アップグレード"}
                  </button>
                </div>
              );
            })}
    ...
    <main style={{ minHeight: "100vh", backgroundColor: "#0e1117", color: "#e8eaf0", paddingBottom: 100 }}>
      <div style={{ maxWidth: 420, margin: "0 auto", padding: "48px 20px 24px" }}>
        ...
        <div style={CS}>
          ...
          {periodEnd && <p style={{ fontSize: 12, color: "#6a7080", margin: "4px 0 14px" }}>次回請求: {periodEnd}</p>}
          {!isPaid && <p style={{ fontSize: 12, color: "#6a7080", margin: "4px 0 14px" }}>月30クレジット・3エージェント</p>}
