    ...
    return (
      <div style={{ maxWidth: 420, margin: "0 auto", padding: "0 0 100px" }}>
        ...
        {copySuccess ? (
          ...
          <button onClick={() => router.push("/login")} style={{ width: "100%", padding: "14px", borderRadius: 10, border: "none", background: "#6c71e8", color: "#fff", fontSize: 15, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
            {"ログインして使う"}
          </button>
        ) : !hasAccess ? (
  ...
  if (selected) {
    ...
    return (
      <div style={{ maxWidth: 420, margin: "0 auto", padding: "0 0 100px" }}>
        ...
        {!isLoggedIn && vars.length > 0 && (
          <div style={{ background: "#1c2028", border: "1px solid #2e3440", borderRadius: 12, padding: "14px 20px", marginBottom: 12 }}>
            <p style={{ fontSize: 13, color: "#6a7080", margin: 0 }}>{"📝 " + vars.length + "個のカスタマイズ項目があります（ログイン後に設定）"}</p>
          </div>
        )}
