...
export const metadata: Metadata = {
  ...
  description: '副業を始めたいなら、Lattice Work。AIが全部教えてくれる。無料で始められる。怪しい副業講座・高額商材は不要。',
  keywords: ['AI副業', '副業 始め方', 'ChatGPT 副業', 'AI 稼ぐ', '副業 初心者'],
}

const WORK_CATEGORIES = [
  { id: 'writing', icon: '✍', title: 'AIライティング副業', desc: 'ブログ記事・LP・SNS投稿をAIで高速作成。クラウドワークスで案件を受注する。', income: '月3〜15万円', difficulty: '★☆☆', time: '今日から', color: '#81C784' },
  { id: 'consulting', icon: '💼', title: 'AI活用コンサル', desc: '中小企業にAI活用を教える。ChatGPTの使い方を教えるだけで報酬が発生する。', income: '月5〜30万円', difficulty: '★★☆', time: '1週間で', color: '#CE93D8' },
  { id: 'image', icon: '[画]', title: 'AI画像生成副業', desc: 'AIで画像を生成してSNS素材・アイコン・バナーとして販売する。', income: '月2〜10万円', difficulty: '★☆☆', time: '今日から', color: '#4FC3F7' },
  { id: 'translation', icon: '[翻]', title: 'AI翻訳・ローカライズ', desc: 'AIで翻訳してから人間が校正。単純翻訳より高単価で受注できる。', income: '月3〜12万円', difficulty: '★☆☆', time: '今日から', color: '#FFD54F' },
  { id: 'prompt', icon: '⚡', title: 'プロンプト販売', desc: '自分が作ったプロンプトをLatticeで販売。収益の80%があなたに入る。', income: '月1〜50万円', difficulty: '★★☆', time: '今すぐ', color: '#FF8A65' },
  { id: 'data', icon: '[分]', title: 'AIデータ分析', desc: 'ExcelデータをAIで分析してレポート作成。企業から高単価で受注できる。', income: '月5〜20万円', difficulty: '★★★', time: '2週間で', color: '#4DB6AC' },
]

const STEPS = [
  { num: '01', title: '副業を選ぶ', desc: '自分に合った副業スタイルを選ぶ。全部無料で試せる。' },
...
export default async function WorkLPPage() {
  const featuredPrompts = await prisma.agent.findMany({
    where: { price: { gt: 0 }, category: { in: ['Writing', 'Business', 'Research'] } },
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#080b14', color: '#e8eaf0', fontFamily: "'DM Sans', 'Hiragino Sans', sans-serif" }}>
      <style>{`.work-card { transition: border-color 0.15s, transform 0.15s; } .work-card:hover { border-color: #3b82f655 !important; transform: translateY(-2px); } .prompt-card { transition: border-color 0.15s; } .prompt-card:hover { border-color: #3b82f655 !important; }`}</style>
      <Nav />
      ...
      <section style={{ position: 'relative', textAlign: 'center', padding: '96px 24px 80px', maxWidth: 860, margin: '0 auto' }}>
        ...
        <p style={{ fontSize: 14, color: '#4a5068', maxWidth: 480, margin: '0 auto 40px', lineHeight: 1.75 }}>
          高額な副業講座も怪しい情報商材も不要。<br />AIプロンプト・手順・テンプレート、全部ここにある。
        </p>
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
          <Link href="/work/space" style={{ background: '#34d399', color: '#080b14', textDecoration: 'none', padding: '14px 32px', borderRadius: 10, fontSize: 15, fontWeight: 900, display: 'inline-block' }}>
            無料でスペースを作る →
          </Link>
          ...
          {WORK_CATEGORIES.map(cat => (
            <div key={cat.id} className="work-card" style={{ background: '#0d1120', border: '1px solid #1c2136', borderRadius: 14, padding: '24px' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, marginBottom: 16 }}>
                <div style={{ width: 48, height: 48, background: cat.color + '18', border: `1px solid ${cat.color}30`, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, flexShrink: 0, color: cat.color, fontWeight: 700 }}>
                  {cat.icon}
                </div>
                <div>
              ...
              <div style={{ display: 'flex', gap: 16, fontSize: 12, borderTop: '1px solid #1c2136', paddingTop: 14 }}>
                <div><span style={{ color: '#4a5068' }}>想定収益 </span><span style={{ color: '#34d399', fontWeight: 700 }}>{cat.income}</span></div>
                <div><span style={{ color: '#4a5068' }}>難易度 </span><span style={{ color: '#fbbf24', fontWeight: 700 }}>{cat.difficulty}</span></div>
                <div><span style={{ color: '#4a5068' }}>開始 </span><span style={{ color: '#e8eaf0', fontWeight: 700 }}>{cat.time}</span></div>
              </div>
            </div>
          ))}
        </div>
      ...
      <section style={{ textAlign: 'center', padding: '80px 24px' }}>
        <h2 style={{ fontSize: 28, fontWeight: 900, letterSpacing: '-0.03em', marginBottom: 12 }}>副業の第一歩を、今日踏み出す。</h2>
        <p style={{ color: '#8b92a9', fontSize: 14, marginBottom: 32, maxWidth: 400, margin: '0 auto 32px' }}>
          高額講座も情報商材も不要。AIと一緒に、今すぐ始めよう。
        </p>
        <Link href="/work/space" style={{ background: '#34d399', color: '#080b14', textDecoration: 'none', padding: '14px 32px', borderRadius: 10, fontSize: 15, fontWeight: 900, display: 'inline-block' }}>
          無料でスペースを作る →
        </Link>
